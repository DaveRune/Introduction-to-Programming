...
namespace octet {
  class play_shader : public shader {
    ...
    void init() {
      ...
      const char fragment_shader[] = SHADER_STR(
        ...
        void Disc()
        {
          ...
          float pixelSum = pixelPosition.x + pixelPosition.y;

          if(distance > radius * tan(sinMultiplier * pixelSum))
          {
            discard;
          }
        }
      ...
      sinMultiplierIndex_ = glGetUniformLocation(program(), "sinMultiplier");
    }

    void render(const mat4t &modelToProjection, int sampler, int resX, int resY, float sinMultiplier) {
      ...
      glUniform1f(sinMultiplierIndex_, sinMultiplier);
